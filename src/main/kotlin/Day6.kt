val input = readFile("Day6_input.txt").joinToString("")

const val PACKET = 4
const val MESSAGE = 14

fun isMarker(position: Int, input: String, searchMode: Int): Boolean {
    var start = position - searchMode
    if (start < 0) start = 0
    val chunk = input.substring(start, position)
    if (DEBUG) println("position: $position; chunk: $chunk")
    if (chunk.toCharArray().toSet().size < searchMode) return false else return true
}


fun main() {
    val packetPosition: Int? = IntRange(1, input.length).asSequence()
        .map { position -> Pair(position, isMarker(position, input, PACKET)) }
        .find { it.second }
        ?.first
    println(println("start-of-packet marker detected; processed $packetPosition characters"))

    val messagePosition: Int? = IntRange(1, input.length).asSequence()
        .map { position -> Pair(position, isMarker(position, input, MESSAGE)) }
        .find { it.second }
        ?.first
    println(println("start-of-message marker detected; processed $messagePosition characters"))
}
