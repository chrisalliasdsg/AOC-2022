...
fun splitContent(content: String): Pair<List<Char>, List<Char>> =
    Pair(
        content.substring(0, content.length / 2).toCharArray().toList(),
        content.substring(content.length / 2).toCharArray().toList()
    )

fun getPriority(char: Char): Int {
    var adjustment = 96 // lowers   : 97-122 -> 1-26 (-96)
    if (IntRange(65, 90).contains(char.code)) {
        adjustment = 38 // capitals : 65-90 -> 27-52 (-38)
    }
    return char.code - adjustment
}

fun part1() {
    val rucksacks = readFile("Day3_input.txt")
